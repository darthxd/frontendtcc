...
const Dashboard = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="card">
          ...
          <div className="space-y-3">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-medium">{currentUser?.role === 'ROLE_ADMIN' ? 'Administrador' : 'Professor'}</span>
            </div>
          </div>
        </div>
        ...
        <div className="card">
          ...
          <div className="space-y-2">
            <div className="flex items-center">
              <div className={`w-3 h-3 rounded-full mr-3 ${authService.isAdmin() ? 'bg-green-500' : 'bg-gray-300'}`}></div>
              <span className="text-sm text-gray-600">Acesso Administrativo</span>
            </div>
            <div className="flex items-center">
              <div className={`w-3 h-3 rounded-full mr-3 ${authService.isTeacher() ? 'bg-green-500' : 'bg-gray-300'}`}></div>
              <span className="text-sm text-gray-600">Acesso de Professor</span>
            </div>
            <div className="flex items-center">
              <div className={`w-3 h-3 rounded-full mr-3 ${authService.isStudent() ? 'bg-green-500' : 'bg-gray-300'}`}></div>
      ...
      {!authService.isAdmin() && (
        <div className="card">
          <h3 className="text-lg font-medium text-gray-900 mb-4">Acesso Limitado</h3>
          <p className="text-gray-600">
            Você tem acesso limitado ao sistema. Entre em contato com um administrador para solicitar permissões adicionais.
          </p>
        </div>
      )}
    </div>
